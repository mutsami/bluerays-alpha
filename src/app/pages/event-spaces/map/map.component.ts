...
export class MapComponent implements OnInit {
  ...
  ngAfterViewInit() { 
    ...
    this.auth.getEventSpacesListings().pipe(take(1)).subscribe(e=>{ 
    this.mapData = e;
    ...
    .setHTML('<img src="'+marker.gallery[0]?.url+'"  onerror="this.onerror=null;this.src=' + "'" + 'https://via.placeholder.com/300x210?' + "' " + '" alt="'+marker.name+'" /> '+
            '<div class="acr-listing-popup-body"><h5><a title="'+marker.name+'">' + marker.name + '</a></h5> <span class="listing-price"> KES. '+  marker.price +'</span>'+
              '<p class="ellipsis2"><i class="fas fa-map-signs"></i> ' + marker.description + '</p> <div class="location-popup-meta">  <span><h3 class="subtitle" >'+marker.size+' Acres </h3></span>  </div></div>'))
    .addTo(this.map);  
    });
